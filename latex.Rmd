...
- Escape Characters: The following characters `& % $ # _ { } ~ ^ \` have special meaning. To produce the raw character, the first seven of them can be typeset by prepending a backslash; for the other three, use the macros `\textasciitilde`, `\textasciicircum`, and `\textbackslash`.

- For inline formulas, enclose the formula in `$`, e.g. $2\times 3\pm 5=6\pm 5$ by `$2\times 3\pm 5=6\pm 5$`. For displayed formulas, use `$$`: get $$\int_{-\infty}^\infty e^{-x^2}\,\text{d}x=\sqrt{\pi}$$ by `$$\int_{-\infty}^\infty e^{-x^2}\,\text{d}x=\sqrt{\pi}$$`.

- For superscripts and subscripts, use `^` and `_`; e.g. $x_i^2$ by `x_i^2`, $\log_2 x$ by `\log_2 x`.
...
- Parentheses. `()[]` make parentheses and brackets. Use `\{` and `\}` for curly braces `{}`.
  ...
  - There are also invisible parentheses, denoted by `.` e.g. $\left.\frac12\right\rbrace$ by `\left.\frac12\right\rbrace`. 
...
- Radical signs / roots: Use `\sqrt`, which adjusts to the size of its argument. For complicated expressions, consider using `{...}^{1/2}` instead.

- Some special functions such as "lim", "sin", "max", "ln", and so on are normally set in roman font instead of italic font. Use `\lim`, `\sin`, etc. to make these: $\lim$, $\sin$, not $lim$, $sin$ !
  - Use subscripts to attach a notation to `\lim`: e.g. $\lim_{x\to 0}$ by `\lim_{x\to 0}`

- Putting extra literal spaces into formulas will not change the amount of space LaTeX puts in: a␣b and a␣␣␣␣b are both ab. To add more space, use `\,` for a thin space, `\;` for a wider space, `\quad` and `\qquad` for larger spaces.
  
- To set plain text, use `\text{}`. You can nest `$` inside of `\text{}`, for example to access spaces.

- Accents and diacritical marks: Use `\hat` for a single symbol, `\widehat` for a larger formula; there are `\bar`, `\overline`, `\vec`, `\overrightarrow` and `\overleftrightarrow`. For dots, use `\dot` and `\ddot`.
...
- Array-Like Environment. Using `&` as seperator between row elements; proceed to next row by issuing `\\`. 
