...
* A Glimpse of the `Gapminder` data
...
- **GDP per Capita** gets mapped to the `x`-position `aes`thetic of the points
- **Life Expectancy** gets mapped to the `y`-position `aes`thetic of the points
- **Population** gets mapped to the `size` `aes`thetic of the points
- **Continent** gets mapped to the `color` `aes`thetic of the points

```{r summary-table-gapminder, echo=FALSE, purl=FALSE}
tibble(`data variable` = c("GDP per Capita", "Life Expectancy", "Population", "Continent"),
       aes = c("x", "y", "size", "color"),
       geom = c("point", "point", "point", "point")) %>%
    kable(caption = "Summary of the grammar of graphics for this plot",
          booktabs = TRUE, linesep = "", format = "html") %>%
    kable_styling(font_size = 16) %>%
    as.character() %>%
    display_html()
```
...
- minimal components:
  ...
  - add *layers* to the plot using the `+` sign: `geom`etric object -- points, lines, bars, the plot title, axes labels, visual themes for the plots, facets, etc
...
> Scatterplots, also called *bivariate plots*, allow one to visualize the *relationship* between two numerical variables
...
- Within `ggplot()`, we specify two of the components of the grammar of graphics as arguments:
...
- Add a layer to the `ggplot()` function call using the `+` sign 
- The added layer specifies the third component of the grammar: the `geom`etric object (here are points by specifying `geom_point()`)
...
- Illustrate linegraphs using another dataset in the `nycflights13` package: the `weather` data frame
  ...
  - `temp`: hourly temperature recordings in Fahrenheit at weather stations near all three major airports in New York City
...
Take a look at both the `weather` and `early_january_weather` data frames by running `View(weather)` and `View(early_january_weather)` in the console. In what respect do these data frames differ?
...
- The bin for the 30-40&deg;F range has a height of around 5000: around 5000 of the hourly temperature recordings are between 30&deg;F and 40&deg;F
- The bin for the 40-50&deg;F range has a height of around 4300: around 4300 of the hourly temperature recordings are between 40&deg;F and 50&deg;F
- The bin for the 50-60&deg;F range has a height of around 3500: around 3500 of the hourly temperature recordings are between 50&deg;F and 60&deg;F
...
1. R chooses `bin = 30` by default
1. For one row has a missing `NA` value for `temp`, it was omitted from the histogram
...
- Suppose we were interested in looking at how the histogram of hourly temperature recordings at the three NYC airports in each month
...
- The "whisker" of these plots extend out from the bottoms and tops of the boxes and represent points less than the 25th percentile and greater than the 75th percentiles, respectively 
...
- The `fill` aesthetic corresponds to the color used to fill the bars, while the `color` aesthetic corresponds to the color of the outline of the bars 
  - This is identical to how we added color to our histogram before: we set the outline of the bars to white by setting `color = "white"` and the colors of the bars to blue steel by setting `fill = "steelblue"`
